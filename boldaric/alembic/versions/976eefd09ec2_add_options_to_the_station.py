...
def upgrade() -> None:
    ...
    op.add_column(
        "stations",
        sa.Column("replay_song_cooldown", sa.Integer(), nullable=True, default=50),
    )
    op.add_column(
        "stations",
        sa.Column("replay_artist_downrank", sa.Float(), nullable=True, default=0.995),
    )
    op.add_column(
        "stations", sa.Column("ignore_live", sa.Boolean(), nullable=True, default=False)
    )

    # Set default values for existing rows
    op.execute(
        "UPDATE stations SET replay_song_cooldown = 50 WHERE replay_song_cooldown IS NULL"
    )
    op.execute(
        "UPDATE stations SET replay_artist_downrank = 0.995 WHERE replay_artist_downrank IS NULL"
    )
